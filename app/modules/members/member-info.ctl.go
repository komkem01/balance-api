...
func (c *Controller) InfoMemberController(ctx *gin.Context) {
	...
	res, err := c.svc.InfoMember(ctx, &InfoRequestService{ID: req.ID})
	if err != nil {
		...
		_ = base.InternalServerError(ctx, "member-info-failed", nil)
		return
	}

	_ = base.Success(ctx, res, "member-info")
}
